...
func (s *GeneratorSuite) TestGeneratorDoesntFollowAlias() {
	expected := `// Aliased is an autogenerated mock type for the Aliased type
type Aliased struct {
	mock.Mock
}

// Param provides a mock function with given fields: a
func (_m *Aliased) Param(a test.Alias) bool {
	ret := _m.Called(a)

	var r0 bool
	if rf, ok := ret.Get(0).(func(test.Alias) bool); ok {
		r0 = rf(a)
	} else {
		r0 = ret.Get(0).(bool)
	}
...
// Return provides a mock function with given fields: b
func (_m *Aliased) Return(b bool) test.Alias {
	ret := _m.Called(b)

	var r0 test.Alias
	if rf, ok := ret.Get(0).(func(bool) test.Alias); ok {
		r0 = rf(b)
	} else {
		r0 = ret.Get(0).(test.Alias)
	}
...
`
	s.checkGeneration(
		filepath.Join(fixturePath, "alias_type.go"), "Aliased",
		false, "", expected,
	)
}
